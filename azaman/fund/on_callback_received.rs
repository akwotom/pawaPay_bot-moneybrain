...
use crate::azaman::{TransactionType, fund::db, gateway, on_transaction_complete};
...
pub fn on_callback_received(id: String) {
    tokio::spawn(async move {
        let mut txn = match db::find_by_id(id.clone()).await {
            Option::None => {
                println!(
                    "Could not process a callback for an azaman fund transaction, because nothing was found for {id}"
                );
                return;
            }
            Option::Some(v) => v,
